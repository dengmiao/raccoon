...
/**
 * @title: CaptchaMessageHelper
 * @description: 短信/邮箱验证码发送、校验
 * @author: dengmiao
 * @create: 2019-05-23 17:39
 **/
public class CaptchaMessageHelper {
    ...
    public CaptchaMessageHelper(CaptchaProperties captchaProperties) {
        ...
        String source = StrUtil.isBlank(sms.getCharSource()) ? sms.getCharSource() : "0123456789";
    ...
    public CaptchaResult generateMobileCaptcha(String mobile, String captchaCachePrefix) {
        ...
        redisHelper.strSet(captchaCachePrefix + ":" + KEY_CAPTCHA + ":" + captchaKey, group, expire, TimeUnit.MINUTES);
        redisHelper.strSet(captchaCachePrefix + ":" + KEY_INTERVAL + ":" + mobile, String.valueOf(System.currentTimeMillis()), interval, TimeUnit.SECONDS);
        if (limitTime > 0) {
            redisHelper.strIncrement(captchaCachePrefix + ":" + KEY_LIMIT_TIME + ":" + mobile, 1L);
            redisHelper.setExpire(captchaCachePrefix + ":" + KEY_LIMIT_TIME + ":" + mobile, limitInterval, TimeUnit.HOURS);
        }
    ...
    public CaptchaResult generateEmailCaptcha(String email, String captchaCachePrefix) {
        ...
        redisHelper.strSet(captchaCachePrefix + ":" + KEY_CAPTCHA + ":" + captchaKey, group, expire, TimeUnit.MINUTES);
        redisHelper.strSet(captchaCachePrefix + ":" + KEY_INTERVAL + ":" + email, String.valueOf(System.currentTimeMillis()), interval, TimeUnit.SECONDS);
        if (limitTime > 0) {
            redisHelper.strIncrement(captchaCachePrefix + ":" + KEY_LIMIT_TIME + ":" + email, 1L);
            redisHelper.setExpire(captchaCachePrefix + ":" + KEY_LIMIT_TIME + ":" + email, limitInterval, TimeUnit.HOURS);
        }
    ...
    private CaptchaResult checkCaptchaWithNumber(String captchaKey, String captcha, String number, String captchaCachePrefix, boolean cacheCheckResult) {
        String group = redisHelper.strGet(captchaCachePrefix + ":" + KEY_CAPTCHA + ":" + captchaKey);
        redisHelper.delKey(captchaCachePrefix + ":" + KEY_CAPTCHA + ":" + captchaKey);
    ...
    public CaptchaResult checkLastResult(String lastCheckKey, String captchaCachePrefix) {
        String lastCheckResult = redisHelper.strGet(captchaCachePrefix + ":" + KEY_CHECK_RESULT + ":" + lastCheckKey);
        CaptchaResult result = new CaptchaResult();
    ...
    public CaptchaResult cacheCheckResult(String cachePrefix, String result) {
        ...
        redisHelper.strSet(cachePrefix + ":" + KEY_CHECK_RESULT + ":" + lastCheckKey, result, expire * 2L, TimeUnit.MINUTES);
    ...
    private boolean checkInterval(String number, String captchaCachePrefix, CaptchaResult result) {
        ...
        String intervalTime = redisHelper.strGet(key);
